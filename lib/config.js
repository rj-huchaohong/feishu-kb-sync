...
function loadConfig() {
  ...
  try {
    ...
    return {
      ...
      spaces: raw.spaces || {},
      root_source: raw.root_source || 'legacy',
      root_updated_at: raw.root_updated_at || null,
    };
  } catch (err) {
...
async function setRoot([rootPath]) {
  ...
  cfg.root = resolved;
  cfg.root_source = 'explicit';
  cfg.root_updated_at = new Date().toISOString();
  saveConfig(cfg);
...
async function list(args = []) {
  ...
  if (args.length > 0 && !json) throw new Error('用法: feishu-kb-sync config list [--json]');
  if (json) {
    const result = {
      ok: true,
      config_file: CONFIG_FILE,
      cache_root: cfg.root,
      root_source: cfg.root_source,
      root_updated_at: cfg.root_updated_at,
      knowledge_bases: Object.entries(cfg.spaces || {}).map(([name, spaceId]) => ({
        knowledge_base: name,
  ...
  if (cfg.root_source) console.log(`根目录来源: ${cfg.root_source}`);
