...
function prepareCacheDir(root, knowledgeBaseName, spaceId) {
  ...
  return { dir, migratedFrom, legacyPresent: !samePath && fs.existsSync(legacy) };
}
...
function managedArtifactRoots(dir) {
  return [path.join(dir, 'docs'), path.join(dir, 'text')];
}

function isManagedArtifact(dir, file) {
  return Boolean(file) && managedArtifactRoots(dir).some((root) => isPathInside(root, file));
}

function removeArtifact(dir, file) {
  if (!file || !isManagedArtifact(dir, file)) return false;
...
function removeEntryArtifacts(entry, dir, keep = new Set()) {
  let removed = 0;
  for (const file of [entry?.local_path, entry?.text_path]) {
    if (file && !keep.has(path.resolve(file)) && removeArtifact(dir, file)) removed++;
  }
  return removed;
}

function removeObsoleteArtifacts(previous, current, dir) {
  const keep = new Set([current?.local_path, current?.text_path].filter(Boolean).map((file) => path.resolve(file)));
  return removeEntryArtifacts(previous, dir, keep);
}

function removeEmptyDirs(dir) {
  for (const root of managedArtifactRoots(dir)) {
...
function moveArtifact(dir, from, to) {
  if (!from || !to || path.resolve(from) === path.resolve(to)) return false;
  if (!isManagedArtifact(dir, from) || !isManagedArtifact(dir, to)) {
...
function reconcileCachedPaths(previous, desired, dir) {
  moveArtifact(dir, previous?.local_path, desired.local_path);
  moveArtifact(dir, previous?.text_path, desired.text_path);
}
...
function buildNodePathInfo(nodes, dir) {
  ...
  for (const [token, node] of Object.entries(nodes)) {
    ...
    if (!node.has_child) {
      if (isTextOnlineNode(node)) {
        item.text_path = path.join(textDir, textNameForOnline(fileName, node));
      } else if (!isOnlineNode(node)) {
        item.local_path = path.join(docsDir, fileName);
        const ext = path.extname(fileName).toLowerCase();
        if (ext === '.pdf') item.text_path = path.join(textDir, replaceExtension(fileName, '.txt'));
        else if (ext === '.docx' || ext === '.doc') item.text_path = path.join(textDir, replaceExtension(fileName, '.md'));
      }
    }
    info[token] = item;
  }
...
function artifactsExist(entry, node, info) {
...
function needsSync(previous, node, info, force) {
  ...
  return !artifactsExist(previous, node, info);
}
...
async function extractFile(filePath, node, textPath) {
...
async function downloadNode(node, info, dir, previous, client = http) {
...
function createStagingDir(dir) {
  const stage = path.join(
    path.dirname(dir),
    `.feishu-kb-sync-staging-${process.pid}-${Date.now()}-${crypto.randomUUID()}`,
  );
  fs.mkdirSync(stage, { recursive: true });
  return stage;
}

function cloneCacheDir(source, target) {
  ...
  for (const entry of fs.readdirSync(source, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      cloneCacheDir(sourcePath, targetPath);
    } else if (entry.isFile()) {
      fs.mkdirSync(path.dirname(targetPath), { recursive: true });
      fs.copyFileSync(sourcePath, targetPath);
    } else if (entry.isSymbolicLink()) {
      throw new Error(`缓存目录不支持符号链接: ${sourcePath}`);
    }
  }
}
...
/**
 * 用同一父目录下的 rename 完成缓存提交，避免把半成品暴露给查询方。
 * 旧目录会在新目录就位后删除；提交失败时尽量恢复旧目录。
 */
function commitCacheDir(stageDir, targetDir) {
  const backupDir = `${targetDir}.previous-${process.pid}-${Date.now()}-${crypto.randomUUID()}`;
  let movedOld = false;
  try {
    ...
    if (movedOld) cleanupDirectory(backupDir);
  } catch (err) {
    if (movedOld && !fs.existsSync(targetDir) && fs.existsSync(backupDir)) {
      try { fs.renameSync(backupDir, targetDir); } catch (_) {}
    }
    throw err;
  } finally {
    cleanupDirectory(stageDir);
  }
}

function prepareStagedManifest(stageDir, actualDir, prepared) {
  const manifestFile = path.join(stageDir, 'manifest.json');
  if (!fs.existsSync(manifestFile)) return;
  const manifest = readManifest(stageDir);
  if (prepared.migratedFrom) remapManifestPaths(manifest, prepared.migratedFrom, stageDir);
  remapManifestPaths(manifest, actualDir, stageDir);
  writeJson(manifestFile, manifest);
}

/**
 * 同步事务包装：所有下载、提取、目录移动和 manifest/tree 写入都先发生在
 * 临时缓存目录中；只有完整流程成功后才替换正式缓存目录。
 */
async function syncSpace(options) {
  if (options.transactional === false) return syncSpaceAtDir(options);
  ...
  const prepared = prepareCacheDir(root, kbName, spaceId);
  const dir = prepared.dir;
  if (prepared.legacyPresent) {
    logger.warn?.(`⚠️ 检测到旧 token 缓存目录仍存在: ${legacyCacheDir(root, spaceId)}；当前同步使用 ${dir}`);
  }

  const stageDir = createStagingDir(dir);
  try {
    cloneCacheDir(dir, stageDir);
    prepareStagedManifest(stageDir, dir, prepared);
    const result = await syncSpaceAtDir({
      ...options,
      cachePath: stageDir,
      prepared: { ...prepared, migratedFrom: null, legacyPresent: false },
    });

    if (result.counts.failed > 0) {
      const error = new Error(`同步存在 ${result.counts.failed} 个失败项，已保留上一份完整缓存`);
      error.code = 'SYNC_PARTIAL_FAILURE';
      error.result = result;
      throw error;
    }

    const manifest = remapManifestPaths(result.manifest, stageDir, dir);
    writeJson(path.join(stageDir, 'manifest.json'), manifest);
    commitCacheDir(stageDir, dir);
    return { ...result, dir, manifest };
  } catch (err) {
    cleanupDirectory(stageDir);
    throw err;
  }
...
async function syncSpaceAtDir({ kbName, spaceId, root, force = false, client = http, logger = console, cachePath = null, prepared: suppliedPrepared = null, onProgress = null }) {
  ...
  const pathInfo = buildNodePathInfo(nodes, dir);
  const nextDocs = {};
  const todo = [];
  let addedCount = 0;
  let changedCount = 0;
  ...
  if (result.complete) {
    for (const [token, previous] of Object.entries(previousDocs)) {
      ...
      try {
        removeEntryArtifacts(previous, dir);
        deletedCount++;
  ...
  for (const [token, node] of Object.entries(nodes)) {
    ...
    if (node.has_child) {
      if (previous && !previous.container) removeEntryArtifacts(previous, dir);
      nextDocs[token] = makeContainerEntry(node, info);
      continue;
    }

    const desired = expectedPaths(node, info);
    if (needsSync(previous, node, info, force)) {
    ...
    try {
      if (previous && pathsChanged(previous, desired)) {
        reconcileCachedPaths(previous, desired, dir);
        movedCount++;
  ...
  const results = await mapLimit(todo, Math.max(CONCURRENCY.ONLINE, CONCURRENCY.DOWNLOAD), async (task) => {
    try {
      const entry = await downloadNode(task.node, task.info, dir, task.previous, client);
  ...
  return {
    dir,
    tree: tree,
    manifest: { ...manifest, complete: result.complete, docs: nextDocs },
    counts,
    changes: summary,
    summary,
  };
