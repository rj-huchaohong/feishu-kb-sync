 ...
 *   feishu-kb-sync package-skill [--output <ZIP??>] [--source <sync??>]
 */
...
async function run(argv) {
  ...
  if (cmd === 'package-skill') {
    if (sub === 'help' || sub === '--help' || sub === '-h') {
      console.log('feishu-kb-sync package-skill [--output <ZIP??>] [--source <sync??>]');
      return;
    }
    await packager.packageSkill(sub ? [sub, ...rest] : rest);
    return;
  }
...
用法:
  ...
  package-skill [--output <ZIP路径>]
                                 create a local package in the current directory for manual copying
`);
}
